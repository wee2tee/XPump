    ...
    public partial class DialogDbConfig : Form
    {
        ...
        private MySqlCreateResult CreateNewMysqlDB(DbConnectionConfig local_config)
        {
            ...
            try
            {
                ...
                //cmd.CommandText += "`cmpldat` DATE NULL,";
                ...
                //cmd.CommandText += "`vatdat` DATE NULL,";
                //cmd.CommandText += "`vatprd` DATE NULL,";
                ...
                //cmd.CommandText += "`reserve` DATE NULL,";
                //cmd.CommandText += "`creby` VARCHAR(8) NOT NULL,";
                //cmd.CommandText += "`credat` DATETIME NOT NULL,";
                //cmd.CommandText += "`userid` VARCHAR(8) NOT NULL,";
                //cmd.CommandText += "`chgdat` DATETIME NOT NULL,";
                //cmd.CommandText += "`userprn` VARCHAR(8) NOT NULL,";
                //cmd.CommandText += "`prndat` DATETIME NULL,";
                //cmd.CommandText += "`prncnt` INT(2) NOT NULL,";
                ////cmd.CommandText += "`prntim` VARCHAR(8) NOT NULL,"; // V.1
                //cmd.CommandText += "`authid` VARCHAR(8) NOT NULL,";
                //cmd.CommandText += "`approve` DATETIME NULL,";
                //cmd.CommandText += "`ponum` VARCHAR(25) NOT NULL,";
                //cmd.CommandText += "`billto` VARCHAR(10) NOT NULL,";
                //cmd.CommandText += "`orgnum` INT(5) NOT NULL,";
                //cmd.CommandText += "`c_type` VARCHAR(4) NOT NULL,";
                //cmd.CommandText += "`c_date` DATE NULL,";
                //cmd.CommandText += "`c_ref` VARCHAR(15) NOT NULL,";
                //cmd.CommandText += "`c_rate` DECIMAL(14, 4) NOT NULL,";
                //cmd.CommandText += "`c_fixrate` VARCHAR(1) NOT NULL,";
                //cmd.CommandText += "`c_ratio` DECIMAL(14, 4) NOT NULL,";
                //cmd.CommandText += "`c_amount` DECIMAL(14, 4) NOT NULL,";
                //cmd.CommandText += "`c_disc` VARCHAR(10) NOT NULL,";
                //cmd.CommandText += "`c_discamt` DECIMAL(14, 4) NOT NULL,";
                //cmd.CommandText += "`c_aftdisc` DECIMAL(14, 4) NOT NULL,";
                //cmd.CommandText += "`c_advamt` DECIMAL(14, 4) NOT NULL,";
                //cmd.CommandText += "`c_total` DECIMAL(14, 4) NOT NULL,";
                //cmd.CommandText += "`c_netamt` DECIMAL(14, 4) NOT NULL,";
                //cmd.CommandText += "`c_netval` DECIMAL(14, 4) NOT NULL,";
                //cmd.CommandText += "`c_ivcamt` DECIMAL(14, 4) NOT NULL,";
                ...
                //cmd.CommandText += "`num2` DECIMAL(14, 4) NOT NULL,";
                //cmd.CommandText += "`str1` VARCHAR(30) NOT NULL,";
                //cmd.CommandText += "`str2` VARCHAR(50) NOT NULL,";
                //cmd.CommandText += "PRIMARY KEY (`id`),";
        ...
        public static void UpdateDbVer(MainForm main_form, DbConnectionConfig local_config)
        {
            try
            {
                var conn_info = "Server=" + local_config.servername + ";Port=" + local_config.port.ToString() + ";Uid=" + local_config.uid + ";Pwd=" + local_config.passwordhash.Decrypted() + ";Character Set=utf8";
                ...
                using (MySqlConnection conn = new MySqlConnection(conn_info))
                {
                    ...
                    {
                        // Check DB Version
                        cmd.CommandText = "Select `version` From `" + local_config.db_prefix + "_" + local_config.dbname + "`.`dbver` Where `id` = 1";
                        ...
                        // ** Upgrade DB Version to 1.0.0.1 ** //
                        if (version.CompareTo("1.0.0.1") < 0)
                        {
                            // Artrn Table
                            cmd.CommandText = "CREATE TABLE IF NOT EXISTS `" + local_config.db_prefix + "_" + local_config.dbname + "`.`artrn` ";
                            cmd.CommandText += "(`id` INT(11) NOT NULL AUTO_INCREMENT,";
                            ...
                            // Add Credit card , Coupon Receive Method in Istab
                            cmd.CommandText = "Insert into `" + local_config.db_prefix + "_" + local_config.dbname + "`.`istab` (tabtyp, typcod, shortnam, shortnam2, typdes, typdes2, is_shiftsales, is_dayend, creby, cretime, chgby, chgtime) ";
                            cmd.CommandText += "Values ('" + ISTAB_TABTYP.RCV_METHOD + "', 'CR', '', 'Credit Card', 'รับชำระด้วยบัตรเครดิต', 'Receive by Credit Card', 0, 0, '" + main_form.loged_in_status.loged_in_user_name + "', '" + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss", CultureInfo.GetCultureInfo("EN-us")) + "', NULL, NULL), ";
                            cmd.CommandText += "('" + ISTAB_TABTYP.RCV_METHOD + "', 'CP', '', 'Coupon', 'รับชำระด้วยคูปองแทนเงินสด', 'Receive by Coupon', 0, 0, '" + main_form.loged_in_status.loged_in_user_name + "', '" + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss", CultureInfo.GetCultureInfo("EN-us")) + "', NULL, NULL)";
                            cmd.ExecuteNonQuery();

                            cmd.CommandText = "Update `" + local_config.db_prefix + "_" + local_config.dbname + "`.`dbver` Set `version`='1.0.0.1', `chgtime`='" + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss", CultureInfo.GetCultureInfo("EN-us")) + "' Where `id` = 1";
                            cmd.ExecuteNonQuery();
                        }
