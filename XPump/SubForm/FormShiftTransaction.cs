...
namespace XPump.SubForm
{
    public partial class FormShiftTransaction : Form
    {
        ...
        public FormShiftTransaction(MainForm main_form, List<pricelist> price_list)
            : this(main_form)
        {
        ...
        private List<stmasPriceVM> GetStmasPriceVMList()
        {
            using (xpumpEntities db = DBX.DataSet())
            {
                ...
                {
                    item.date = this.price_list.Where(p => p.stmas_id == item.stmas_id).FirstOrDefault() != null ? (DateTime?)this.price_list.Where(p => p.stmas_id == item.stmas_id).First().date : null;
